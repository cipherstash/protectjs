...
const GATEWAY_URL = process.env.CIPHERSTASH_WIZARD_GATEWAY_URL ?? 'http://localhost:8787'
const RUN_INTEGRATION = process.env.WIZARD_INTEGRATION === '1'

describe.skipIf(!RUN_INTEGRATION)('Agent SDK integration (real gateway)', () => {
  beforeAll(async () => {
  ...
  it('sends a prompt and receives a text response', async () => {
    ...
    try {
      let signalDone!: () => void
      const resultReceived = new Promise<void>((r) => { signalDone = r })

      const promptStream = async function* () {
        yield {
          type: 'user' as const,
          session_id: '',
          message: { role: 'user' as const, content: 'Reply with exactly: WIZARD_TEST_OK' },
      ...
      const response = query({
        ...
        options: {
          ...
          tools: [],
          disallowedTools: ['Bash', 'Write', 'Edit', 'Read', 'Glob', 'Grep', 'Agent'],
          env: {
            ...process.env,
  ...
  it('agent uses the Read tool to read a file', async () => {
    ...
    try {
      ...
      expect(collectedText.join(' ')).toContain('CIPHER_STASH_SECRET_VALUE_12345')
    } finally {
      rmSync(tmp, { recursive: true, force: true })
    }
  }, 90_000)
