...
export async function statusCommand() {
  ...
  if (installed) {
    ...
    p.log.warn('EQL is not installed. Run `npx stash db install` to install it.')
  ...
  try {
    ...
    if (message.includes('does not exist')) {
      p.log.info(
        'Active encrypt config: table not found (run `npx stash db push` to create it)',
      )
    } else {
