...
describe('createDrizzleProvider getNextSteps', () => {
  ...
  it('uses npx when package manager is npm', () => {
    ...
    expect(steps[0]).toBe(
      'Set up your database: npx @cipherstash/cli db install --drizzle',
    )
  })
  ...
  it('uses bunx when package manager is bun', () => {
    ...
    expect(steps[0]).toBe(
      'Set up your database: bunx @cipherstash/cli db install --drizzle',
    )
  ...
  it('uses pnpm dlx when package manager is pnpm', () => {
    ...
    expect(steps[0]).toBe(
      'Set up your database: pnpm dlx @cipherstash/cli db install --drizzle',
    )
  })
  ...
  it('uses yarn dlx when package manager is yarn', () => {
    ...
    expect(steps[0]).toBe(
      'Set up your database: yarn dlx @cipherstash/cli db install --drizzle',
    )
  })
})
